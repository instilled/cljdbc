...
(with-connection [conn ds]
  ...
  ;; is set to true. This may also be added to the `parse-statement`
  ;; method.
  (let [rows-affected (jdbc/insert! conn my-update [{:system-old "Solar System" :system-new "Solar System (updated)"}])]
    ;; do something useful
    ))
```
...
savepoints feature. A new transaction scope is requested by
invoking `with-transaction`.

```clojure
(with-transaction [conn conn] <optional-options>
   ...)
```

Note that cljdbc executes all
database ops in an implicit transaction context
